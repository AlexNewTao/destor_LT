    ...
    for (; iter != end; iter = g_sequence_iter_next(iter)) {
        ...
        if (!CHECK_CHUNK(c, CHUNK_DUPLICATE)) {
            ...
            if(id != TEMPORARY_ID){
                ...
                if (id<container_count_start)
                {
                    struct containerMeta* cm = retrieve_container_meta_by_id(c->id);
                    if (!g_hash_table_contains(id_shift_hash,&id))
                    {                                              
                        //struct containerMeta *cm = retrieve_container_meta_by_id(id);
            
                        //g_hash_table_add(id_shift_hash,&id);
                        int64_t* id_shift_hash_id=(int64_t*)malloc(sizeof(int64_t));
                        *id_shift_hash_id=id;
                        g_hash_table_insert(id_shift_hash,id_shift_hash_id,id_shift_hash_id);
                        //g_hash_table_add(id_shift_hash,&id_shift_hash_id);
                        ...
                        is->id_shift_data.container_id=id;
                        is->id_shift_data.chunk_num=container_chunk_number;
                
                        is->id_shift_data.chunk_shift=globle_shift;
                    

                        //_id_shift_AddEnd(is->id_shift_data);
                    ...
                    int32_t index_shift =me->position;
                    //printf("in the index the index_shift is %d\n",index_shift);
            
                    int32_t start_shift=get_shift_by_id(c->id);
                    int32_t n=index_shift+start_shift;

                        //printf("in the index n is %d\n",n );
