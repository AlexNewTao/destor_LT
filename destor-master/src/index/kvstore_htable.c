	...
	if ((fp = fopen(indexpath, "r"))) {
		...
		for (; key_num > 0; key_num--) {
			...
			for (i = 0; i < id_num; i++)
			{
				...
				//int64_t f_id=get_value(kv)[i];
				//int64_t f_fp=get_key(kv)[i];
				

				
				/*struct containerMeta *cm = lru_cache_lookup(is_cache, &f_fp);
                //printf("get containermeta in index!, cm id == %d\n", cm->id);
                if (!cm) {
                    VERBOSE("gc cache: container %lld is missed", f_id);
                    cm = retrieve_container_meta_by_id(f_id);
                    assert(lookup_fingerprint_in_container_meta(cm, &f_fp));
                    lru_cache_insert(is_cache, cm, NULL, NULL);
                }*/
                
/*                if (g_hash_table_lookup(id_shift_hash,&f_id)!=NULL)
                {
			...
			//printf("update the id_shift and id_shift_hash successfully!\n");
