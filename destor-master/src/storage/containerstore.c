...
static void* append_thread(void *arg) {
	...
	while (1) {
        ...
        temp->id=c->meta.id;
        temp->len=(int16_t)rtmlen;

        int16_t *array=(int16_t*)malloc(rtmlen*sizeof(int16_t));
        temp->rtm=array;
        int32_t i;
        int32_t version=CBT[0];
        //printf("version is %d\n",version);
        for (i = 0; i <rtmlen; i++)
        {
            array[i]=(int16_t)version;
        }
...
void init_container_store() {
	...
	if ((fp = fopen(containerfile, "r+"))) {
		...
		//printf("container_count sss is %d\n",container_count_start );
    ...
    //NOTICE("Init reference time map successfully");
}
...
void close_container_store() {
	...
	update_container_bit_table_and_RTM(CBT[0]);
	printf("update_container_bit_table_and_RTM successfully!\n");
...
void write_container_async(struct container* c) {
	...
	//update_id_shift(c->meta.id);
/*	if (g_hash_table_lookup(id_shift_hash,&c->meta.id)!=NULL)
	{
		g_hash_table_add(id_shift_hash,&c->meta.id);
                  

        int container_chunk_number=c->meta.chunk_num;
                    

            //append the the end of id_shift!
        struct _id_shift_type *is = (struct _id_shift_type*)malloc(sizeof(struct _id_shift_type));
        is->id_shift_data.container_id=c->meta.id;
        is->id_shift_data.chunk_shift=globle_shift;
                    

        struct _id_shift_type *ishead=_id_shift_AddEnd(is->id_shift_data);

             
            //move the shift
         globle_shift=globle_shift+container_chunk_number;
         //printf("globle_shift is %d\n",globle_shift);
...
struct metaEntry* get_metaentry_in_container_meta(
